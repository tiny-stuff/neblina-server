...
static Session* sparrot_session_create(SOCKET fd, void* data)
{
    ...
    SParrotSession* psession = (SParrotSession *) CALLOC(1, sizeof(SParrotSession));
...
static auto server_thread_function = [](int n_threads) {
    server_running = true;
    SSLServer* server = ssl_server_create(23457, false, sparrot_session_create, n_threads, &ssl_key);
...
TEST_SUITE("SSL")
{
    TEST_CASE("SSL (server multithreaded, client single threaded)")
    {
        ssl_subsystem_init();
        ...
        SSLClient* client = sslclient_create("127.0.0.1", 23457);
        CHECK(client);
        ...
        sslclient_destroy(client);
        server_running = false;
        server_thread.join();

        ssl_subsystem_finalize();
